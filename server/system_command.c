    ...
    const char *command, struct connection_result *result)
{
    ...
    return
        ...
        IF_ELSE(read_char(&command, '*'),
            parse_eos(&command)  ?:
            get_special_keys(result),
        //else
            parse_name(&command, key, sizeof(key))  ?:
            parse_eos(&command)  ?:
            get_special_value(key, result));
}


static error__t put_special(
    ...
    const char *action, enum change_set *change_set)
{
    ...
    else if (strcmp(action, "SPECIAL" ) == 0) *change_set = CHANGES_SPECIAL;
    else
...
static const struct command_table_entry command_table_list[] = {
    ...
    { "SPECIAL",    true,  .get = get_special,  .put = put_special, },
