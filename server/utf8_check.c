 ...
 * converted to UTF-16, namely surrogate characters (U+D800..U+DFFF) and
 * non-Unicode positions (U+FFFE..U+FFFF).  This routine is very likely to find
 * a malformed sequence if the input uses any other encoding than UTF-8. It
 * therefore can be used as a very effective heuristic for distinguishing
...
/* Lightly edited from original downloaded from
